...
def rem_fillers(ctx):
    ...
    tokens = ctx.split(" ")
    for token in tokens:
        if rem_punc(token.lower()) in fillers:
...
def gen_tscs(test_dataset, tokenizer, length):
    with open(
        "/content/drive/MyDrive/Journal/data/{}.json".format(test_dataset), "r"
    ) as out:
        tscs = json.load(out)
...
def gen_summaries(tscs_preprocessed, summarizer, out_dir):
    s2 = []
    filename = []
    meet_no = 1
    for tsc in tscs_preprocessed:
        ...
        outfile = open("{}/{}.txt".format(out_dir, k), "w")
        outfile.write(s1)
        outfile.close()
        meet_no += 1
...
def gen_summaries_(tscs_preprocessed, summarizer, out_dir):
    s2 = []
    filename = []
    meet_no = 1
    for tsc in tscs_preprocessed:
        k = tsc["meeting_id"]
        attendees = tsc["attendees"]
        print("{}: {}".format(meet_no, k))
        k = k + "_summary"
        filename.append(k)
        v = tsc["segments"]
        ...
        s1 = format_summary_(s1, attendees)
...
def gen_summaries2(tscs_preprocessed, summarizer, summarizer_, out_dir):
    s2 = []
    s2_ = []
    filename = []
    meet_no = 1
