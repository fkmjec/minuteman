    ...
    def process(self):
        ...
        while True:
            ...
            try:
                ...
                print("connection reset -- why?", file=sys.stderr)
                break
...
class MultilingServerProcessor(ServerProcessor):
    def __init__(self, c, online_asr_proc, min_chunk, langfile):
        self.connection = c
        self.online_asr_proc = online_asr_proc
        self.min_chunk = min_chunk
        ...
        self.dict_tokenizers = {  # dict of lan: tokenizer object
            self.online_asr_proc.asr.original_language: self.online_asr_proc.tokenizer
        }

    def lang_changes(self):
        with open(self.langfile, "r") as f:
            lang = f.read().strip()
        if lang not in WHISPER_LANG_CODES:
            print(
                f"language code {lang} in {self.langfile} is not supported by Whisper. Ignoring change.",
                file=sys.stderr,
            )
            return False

        if lang != self.current_lang:
            out = self.online_asr_proc.finish()
            self.current_lang = lang
            print(
                f"Changing the language and resetting. New lang: {self.current_lang}",
                file=sys.stderr,
            )

            toff = self.online_asr_proc.buffer_time_offset
            self.online_asr_proc.asr.original_language = self.current_lang

            # set a tokenizer for the lang -- either reuse old one, or create a new
            if self.current_lang in self.dict_tokenizers:
                tok = self.dict_tokenizers[self.current_lang]
            else:
                tok = create_tokenizer(self.current_lang)
                self.dict_tokenizers[self.current_lang] = tok
            self.online_asr_proc.tokenizer = tok

            self.online_asr_proc.init()
            self.online_asr_proc.buffer_time_offset = toff
            return (True, out)
        return (False, None)
